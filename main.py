...
class DeflectouchWidget(Widget):
    ...
    def fire_button_pressed(self):
        ...
        bullet_position = (tower_position[0] + 48 + cos(tower_angle)
                           * 130, tower_position[1] + 70 + sin(tower_angle) * 130)
    ...
    def level_button_pressed(self):
        ...
        for row in range(5):
            for collumn in range(8):
                button = Button(text=str(row*8 + (collumn + 1)),
    ...
    def level_accomplished(self):
        ...
        image = Image(source='graphics/accomplished.png',
        ...
        # little hack to sleep for 1 sec
        animation += Animation(size=(350, 416), duration=1)
    ...
    def accomplished_animation_complete(self, animation, widget):
        ...
        if self.level != 40:
            if self.level % 8 == 0:
                ...
                Popup(title='New levels unlocked!', content=Label(
                    text='Next 8 levels unlocked!', font_size=18), size_hint=(0.3, 0.15)).open()
    ...
    def reset_level(self):
        ...
        if self.bullet != None:
            ...
            self.bullet.animation.unbind(
                on_complete=self.bullet.on_collision_with_edge)
    ...
    def load_level(self, level):
        ...
        LEVEL_OFFSET = [self.center_x -
                        (LEVEL_WIDTH/2) * BRICK_WIDTH, self.height / 12.5]
        ...
        try:
            level_image = kivy.core.image.Image.load(
                self.app.directory + '/levels/level%02d.png' % level, keep_data=True)
        except Exception as e:
            error_text = 'Unable to load Level %d!\n\nReason: %s' % (level, e)
            Popup(title='Level loading error:', content=Label(
                text=error_text, font_size=18), size_hint=(0.3, 0.2)).open()
            return
    ...
    def build_level(self, instance):
        ...
        if self.level_build_index < len(self.obstacle_list):
            self.background.add_widget(
                self.obstacle_list[self.level_build_index])
        else:
            if self.level_build_index - len(self.obstacle_list) != len(self.goal_list):
                self.background.add_widget(
                    self.goal_list[self.level_build_index - len(self.obstacle_list)])
            else:
...
class Deflectouch(App):
    ...
    def build_config(self, config):
        ...
        config.setdefault('GamePlay', 'Levels',
