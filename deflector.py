...
class Deflector(Scatter):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            Color(.8, .8, .8)
            self.deflector_line = Line(points=(
                self.touch1.x, self.touch1.y - 1, self.touch1.x + self.length, self.touch1.y - 1))
            self.deflector_line2 = Line(points=(
                self.touch1.x, self.touch1.y + 1, self.touch1.x + self.length, self.touch1.y + 1))
        ...
        self.point_pos_origin = [self.point1.x,
                                 self.point1.y, self.point2.x, self.point2.y]
        ...
        self.apply_transform(rotation_matrix, post_multiply=True,
                             anchor=self.to_local(self.touch1.x, self.touch1.y))
    ...
    def size_callback(self, instance, size):
        ...
        self.point1.pos = self.point_pos_origin[0] + (
            40 - size[0])/2, self.point_pos_origin[1] + (40 - size[0])/2
        self.point2.pos = self.point_pos_origin[2] + (
            40 - size[0])/2, self.point_pos_origin[3] + (40 - size[0])/2
    ...
    def collide_widget(self, wid):
        point1_parent = self.to_parent(
            self.point1.center[0], self.point1.center[1])
        point2_parent = self.to_parent(
            self.point2.center[0], self.point2.center[1])
